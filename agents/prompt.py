...
You can ONLY declare "Zero Doubt and Absolutely Confident" if **ALL of the following conditions are *flawlessly and unequivocally* met, without requiring *any* inference, assumption, or interpretation beyond what is explicitly stated or shown**:
(a) **Direct, Explicit, and Unmistakable Visual Evidence**: The image *itself* MUST contain direct, explicit, and unmistakable visual evidence that fully and unambiguously answers the question. If there's any room for interpretation of visual cues, it's "Any Doubt".
(b) **Strict Adherence to Provided Information**: Do NOT use any external knowledge, general knowledge, or learned associations beyond what is explicitly present in the image. If the answer isn't *explicitly and literally* in the provided materials, it's "Any Doubt".
(c) **Rejection of Inferential Reasoning**: If answering the question requires *any* form of inference, logical deduction (beyond simple matching), interpretation of nuanced details, or filling in missing information, you MUST classify it as "Any Doubt". Only direct factual recall from the provided context is permitted for an "Absolutely Confident" answer.
(d) **Literal Match for Question Scope**: The user's question must be answerable *solely* and *literally* either from this direct visual evidence OR from provided  information that *perfectly and literally matches* the visual context and *directly and explicitly answers* the question. No inferential steps are allowed to bridge gaps.
(e) **Pronoun & Reference Clarity Gate**: If the user’s question contains any pronoun or demonstrative reference (e.g., “it,” “they,” “this,” “that”), you MUST be able to trace the pronoun’s or demonstrative’s reference with zero ambiguity and perfect clarity based solely on the provided image or context. If there is any ambiguity or uncertainty in what the pronoun or demonstrative refers to, it defaults to "Any Doubt".
(f) **Absolute Consistency and No Conflicts**: There must be absolutely no conflicting information, no discrepancies, and no ambiguities whatsoever between the image, and the chat history. The slightest hint of inconsistency defaults to "Any Doubt".
...
ORIGIN_API_PROMPT = """The following information may help you better understand the context. 
It provides factual reference details related to the task. 
You can use this information to improve the quality or accuracy of your response, but feel free to ignore it if it is not relevant. 
Since this information is from an image similar to the one you received, it may or may not be related. 
Only use it if you're confident that it is indeed relevant; otherwise, disregard it.
